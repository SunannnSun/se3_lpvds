...
def process_bag_file(path):
    ...
    # output_traj = np.empty((L, 1), dtype=object)  #output pos + vel
    ...
    for l in range(L):
        ...
        # p_in += pos_traj  # output pos only
        p_in += [pos_traj[:, i] for i in range(pos_traj.shape[1])]  # output pos only

        # output_traj[l, 0] = np.vstack((pos_traj[:, 0:-1], vel_traj)) #output pos + vel
    ...
    return np.array(p_in), q_in, index_list
